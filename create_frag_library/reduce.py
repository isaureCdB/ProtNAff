...
This file is part of the ATTRACT project. License: GPL
"""
...
parser.add_argument(
    "--compat", help="Maximize compatibility with reduce.f", action="store_true"
)
parser.add_argument("--chain", help="Set the chain in the output PDB", default=" ")
parser.add_argument(
    "--startres",
    help="Set residue number of the first residue, default 1",
    type=int,
    default=1,
)
parser.add_argument(
    "--startatom",
    help="Set atom number of the first atom, default 1",
    type=int,
    default=1,
)
parser.add_argument(
    "--batch",
    help="run reduce in batch mode. Input and output must be two lists of PDBs",
    action="store_true",
)
...
def print_res(mapping=True):
    ...
    for l in ff[resname]:
        ...
        print(
            "ATOM%7d %4s %3s %s%4d    %8.3f%8.3f%8.3f%5d%8.3f 0%5.2f" % line, file=outp
        )
    rescoor = {}
