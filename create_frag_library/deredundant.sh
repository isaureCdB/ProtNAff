...
#   $m-aa-fit-clust${dr} : global indices of non-redundant fragments
  ...
  $d/concatenate_clusters.py $f-clust$a > $f-clust$a-concat
  $d/fastsubcluster_npy.py $f.npy $f-clust$a-concat $dr $f-noclash-clust$dr /dev/null 2> fastsubcluster-$m-$dr.log
