...
output_superclust = npy_file.split(".npy")[0]+"-superclust"+str(cutoff)
### use select-struct-npy.py instead
#output_numpyclust = npy_file.split(".npy")[0]+"-clust"+str(cutoff)+'.npy'  #cluster hearts
...
for rootclustnr, rootclust in enumerate(rootclusters):
    if not rootclustnr%1000:
        print(rootclustnr, file=sys.stderr)
    if len(rootclust) == 1:
...
#numpyclust = clust_struc[ np.array([c[0]-1 for c in subclust]) ]
#np.save(output_numpyclust, numpyclust)
