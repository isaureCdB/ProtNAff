...
def query_ss(pdb_info, chain_id, length):
    ...
    nucl_ss = [n for n in ss if ss[n][0] in ss_set]
...
def main():
    js = json.load(open("/amoniot/Documents/x3dna.json"))
    for pdb_id in js:
        pdb_info = js[pdb_id]
        ...
        for chain_id in pdb_info['nachains']:
            ...
            if len(nuclfrag) > 0:
                for i in range(len(nuclfrag) - 1):
                    if abs(nuclfrag[i] - nuclfrag[i + 1]) > 2 and len(nuclfrag) < 25:
                        print(pdb_id, nuclfrag)
