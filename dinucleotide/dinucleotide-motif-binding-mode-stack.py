"""For a dinucleotide, defined by the first two nucleotides in a 0.2A trinucleotide cluster,
 ...
 Once identified, apply the rotation/translation matrices to the protein stacking residues.
"""
...
for motif in motifs:
    ...
    for frag in frags2:
        struc = frag["structure"].decode()
        if struc in pdb_list:
            pdb_codes.add(struc)
    frags[motif] = frags2
...
for code in pdb_codes:
    threedna_file = f"database/3dna/{code}-1-dssr.json"
    if not os.path.exists(threedna_file):
        continue
    with open(threedna_file) as fp:
        d = json.load(fp)
    threedna[code] = d
...
for n, close_frag in enumerate(close_frags):
    ...
    fit_mask = (pdb["resid"] == close_frag["indices"][0]) | (
        pdb["resid"] == close_frag["indices"][1]
    )
    ...
    fname = f"database/brutPDBs/{struc}.pdb"
    with open(fname) as fp:
        pdbtxt = fp.read()
    pdb = parse_pdb(pdbtxt)
    pdb = pdb[pdb["model"]==1]
    ...
    stackmask = np.zeros(len(pdb), bool)
    domain_mask = np.zeros(len(pdb), bool)
    d = threedna[struc]
    for stacking in d["stacks"]:
        ntlist = stacking["nts_long"]
        ntlist2 = ntlist.split(",")
        nts = [x.split(".") for x in ntlist2]
        for nt in nts:
            if nt[2] != chain:
                continue
            if int(nt[4]) not in close_frag["indices"][:2]:
                continue
            break
        else:
            continue
        for nt in nts:
            if nt[3] not in ("TYR", "PHE", "TRP"):
                continue
            prot_resid = int(nt[4])
            currmask = (pdb["chain"] == nt[2].encode()) & (pdb["resid"] == prot_resid)
            curr_domainmask = currmask.copy()
            currmask |= (
                (pdb["chain"] == nt[2].encode())
                & (np.isin(pdb["resid"], list(range(prot_resid - 2, prot_resid + 3))))
                & np.isin(pdb["name"], (b"CA", b"C", b"O", b"N"))
            )
            stackmask |= currmask

            curr_domainmask |= (
                (pdb["chain"] == nt[2].encode())                
                & (pdb["resid"] >= prot_resid - 30)
                & (pdb["resid"] <= prot_resid + 30)
                & np.isin(pdb["name"], (b"CA", b"C", b"O", b"N"))
            )
            domain_mask |= curr_domainmask

    assert stackmask.sum()
    pdb2 = pdb[stackmask]
    pdbtxt = write_pdb(pdb2, ter=False)
    pdbtxt = pdbtxt.replace("TER\n","")
    fname = f"{output_dir}/{struc}_{chain}-{ind}-stack.pdb"
    with open(fname, "w") as fp:
        fp.write(pdbtxt)

    assert domain_mask.sum()
    pdb2 = pdb[domain_mask]
    pdbtxt = write_pdb(pdb2, ter=False)
    pdbtxt = pdbtxt.replace("TER\n","")
    fname = f"{output_dir}/{struc}_{chain}-{ind}-domain.pdb"
    with open(fname, "w") as fp:
        fp.write(pdbtxt)
