...
seqsigs = []
with open("xox-stacking-sorted-seqsig.txt") as fp:
...
for n, seq0 in enumerate(sequences):
    ...
    for seqsig in seqsigs:
        if compare_seqsig(rrm_seqsig, seqsig):
            print("Has XOX double stacking,", sequence_titles[n])
            unique_seqsigs = unique_stack_seqsigs
            nstack += 1
            break
    else:
        print("No XOX double stacking", sequence_titles[n])
...
print()
f1 = "rrm-xox-seqsigs.list"
print(
    f"{nstack} XOX stackings, {len(unique_stack_seqsigs)} unique seqsigs, write to {f1}"
)
with open(f1, "w") as fp:
    for seqsig in unique_stack_seqsigs:
        print(unique_stack_seqsigs[seqsig], seqsig, file=fp)
f2 = "rrm-nonxox-seqsigs.list"
print(
    f"{nnonstack} double aromatics without XOX stackings, {len(unique_nonstack_seqsigs)} unique seqsigs, write to {f2}"
)
