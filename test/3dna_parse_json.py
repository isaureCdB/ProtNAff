...
def initialise_all(d, js):
    ...
    for k in keys:
        if k not in js:
            js[k] = []
    #
    outputs = ["bptype", "intraRNA_hb", "stacking", "ss", "NAprot_hb"] # +"breaks"
    for k in outputs:
        if k not in d:
...
def update_intraRNA(d_cc, rr, code, n, dist, AccDonn):
...
def get_hbonds(js_hbond, d_NAprot_hb, d_intraRNA_hb):
    global chains, prochainss
    for j in js_hbond:
        ...
        aa_indices = [ xr for (xr, x) in enumerate(residues) if x[1] in prochainss ]
        ...
        if j["residue_pair"] == "nt:nt":
            ...
            for (a, b) in [(0, 1), (1, 0)]:
                ...
                d_intraRNA_hb[cc[a]] = update_intraRNA(d_intraRNA_hb[cc[a]], rr[a], code, n, j["distance"], j["donAcc_type"])
        else:
            #"nt:aa" or "aa:nt":
            print(j)
            print(atoms)
    ...
    return d_NAprot_hb, d_intraRNA_hb
...
def get_nonPairs_hbonds(j_nonPairs, d_intraRNA_hb):
    ...
    for hbond in hbonds.split(","):
        ...
        for x in ind_in_chains:
            ...
            d_intraRNA_hb[cc] = update_intraRNA(d_cc, rr, code, neighbors[pos[x]], 1, 0)
...
#
    ...
    #NAprot_hb_sum = Nb of RNA-NA/prot Hb for each nmap part
    #NAprot_hb  = list of RNA-NA/prot Hb for each nmap part
...
for struct in sorted(chainsmodels.keys()):
    ...
    d["NAprot_hb"], d["intraRNA_hb"],  = get_hbonds(js["hbonds"], d["NAprot_hb"], d["intraRNA_hb"])
